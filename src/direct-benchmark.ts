import type { DirectBenchmarkConfig, BenchmarkResult, TimingResult, Stats } from './types.js';
...
export async function runDirectBenchmark(config: DirectBenchmarkConfig): Promise<BenchmarkResult> {
  ...
  // Create the compute instance using the direct provider SDK
  const compute = config.createCompute();

  const results: TimingResult[] = [];

  console.log(`\n--- Benchmarking (direct): ${name} (${iterations} iterations) ---`);
